-- ════════════════════════════════════════════════════════════════════════════
-- Garnet Fund — Risk Dashboard Wave 1
-- Combined migrations 0021 + 0022. Paste into Supabase → SQL Editor → Run.
--
-- Safe to run more than once: every table, index and column is guarded, and
-- policies are dropped before being recreated.
--
-- If the editor rejects the ALTER TYPE on line ~20 with "cannot be executed
-- inside a transaction block", run that one statement on its own first, then
-- run the rest. Nothing below uses the new enum value, so the split is safe.
-- ════════════════════════════════════════════════════════════════════════════
...
-- ── After this runs ─────────────────────────────────────────────────────────
-- Give the Risk Manager their role. Only this role (plus admin/developer) can
-- edit a limit, record a position approval, or acknowledge an alert.
-- Uncomment, set the address, and run:
--
--   UPDATE user_profiles SET role = 'risk_manager' WHERE email = 'cooper@…';
--
-- Verify the tables landed:
--
--   SELECT table_name FROM information_schema.tables
--    WHERE table_schema = 'public'
--      AND table_name IN ('risk_config','risk_config_history','position_approvals',
